...
# from torch.distributions.normal import Normal
#posterior_target=outs.posterior_target        

# emp_pmu = posterior_target.mean(dim=1)
# emp_pstd = posterior_target.std(dim=1)
# normal_dist = Normal(loc=emp_pmu, scale=emp_pstd)                    
# normal_dist

# logprob = normal_dist.log_prob(yt[:,:,None,:]).mean(dim=1)
# #logpro 
# label = logprob.argmax(dim=1).reshape(-1)
# label 
...
def train_epochs_pair(batch_dataset_pair,model,opt,lossfun,current_iter=1,total_iter = 500):    
    ...
    for dataset_pair in batch_dataset_pair:
        ...
        if model.modelname in ['gpdep']:
            ...
            #lambda_reg = compute_lambda(current_iter,total_iter=total_iter,period = args.printfreq)
            #lambda_reg = compute_lambda(current_iter,period = args.printfreq)
            #lambda_reg = compute_lambda(current_iter)
            lambda_reg = compute_lambda(current_iter,period = args.printfreq)            
            lambda_regloss = lambda_reg*outs.regloss             
            #print('lambda_reg,outs.regloss,lambda_regloss')
            #print(lambda_reg,outs.regloss,lambda_regloss)
        ...
        #if current_iter % (2*args.printfreq) == 0  or current_iter== 1:     
        #    print('outs.labels {}'.format(outs.labels))                
...
for i in range(1,args.nepochs + 1):   
    ...
    if i%args.printfreq ==0 or i== 1:                
        ...
        #save_path_set = './regression_task_single/syndata_{}_v{}/dep{}_{}_{}'.format(tasktype,args.datav, dep, testtype, -64)
