...
parser.add_argument('--dataname',type=str,default='waterdepth')
...
def get_model(modelname='gpdep',cnntype='deep'):        
    ...
    #return model.double(),opt,lossfun
...
def train_epochs_pair(dataset_pair,model,opt,lossfun):    
    ...
    #y_mean,y_std = model(context_x.double().cuda(),context_y.double().cuda(),target_x.double().cuda())    
    #obj = lossfun( y_mean,y_std, target_y.double().cuda())
...
saved_modelparam_path = './param_{}/{}_{}_nepochs{}_initl{}.pth'.format(dataname,modelname,cnntype,args.nepochs,args.initl)
...
wandb.run.name = '{}_{}_nepochs{}_initl{}_v10'.format(model.modelname,cnntype,args.nepochs,args.initl)    
wandb.run.save()
# watch model
wandb.watch(model)
...
#                                  ntask=ntask,nbatch=nbatch,ncontext=ncontext,ntarget=ntarget)
    ...
    savepath_set = './realdata_{}_v{}/set_{}'.format(dataname,args.datav,i)    
...
wandb.log({"bestval-intrain": best_loss})
