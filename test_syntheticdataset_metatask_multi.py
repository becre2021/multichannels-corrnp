...
#     return  np.array(likelihoods).mean().round(2)

#def compute_lambda(current_iter,total_iter=500,period=20,rate=0.9): v49
#def compute_lambda(current_iter,total_iter=500,period=20,rate=0.75): v50
#def compute_lambda(current_iter,total_iter=500,period=50,rate=0.5): v50
...
#     #print('mse_regloss {}, nll_regloss {}'.format(mse_regloss.item(),nll_regloss.item()))
#     #outs.regloss = mse_regloss     
#     outs.regloss = nll_regloss 
#     return outs
...
def compute_regloss(yt,outs,nllloss,eps=1e-6,tempering0=1e0):
    ...
    #print(' learnable_logits ,appr_logits')
    #print( learnable_logits ,appr_logits)
...
def train_epochs_pair(batch_dataset_pair,model,opt,lossfun,current_iter=1,total_iter = 500):    
    ...
    for dataset_pair in batch_dataset_pair:
        ...
        #outs = model(context_x.cuda(),context_y.cuda(),target_x.cuda(),iterratio=iterratio)    
        
        obj = lossfun( outs.pymu, outs.pystd, target_y.cuda(), intrain=True) 
        
        #if 'regloss' in outs.keys():
        #    if model.modelname in ['gpdep']:
        #        lambda_reg = args.reglambda
        #        lambda_regloss = lambda_reg*outs.regloss 
        #        obj = obj + lambda_regloss
        #        regloss.append(lambda_regloss.cpu().data.numpy())       
        #        print('lambda_regloss: {}'.format(lambda_regloss))
                
    
        if model.modelname in ['gpdep']:
            #outs = compute_regloss(target_y.cuda(),outs)
...
def get_model(modelname='gp',cnntype = 'shallow'):        
    ...
    if modelname == 'gpind':
        ...
        model = ICGP_Convnp(in_dims=1,out_dims=1,num_channels=num_nchannels,num_postsamples=num_samples,init_lengthscale=init_lscale,cnntype=cnntype).cuda()        
    ...
    if modelname == 'gpdep':
        num_samples = args.ngpsamples            
        model = DCGP_Convnp(in_dims=1,out_dims=1,num_channels=num_nchannels,num_postsamples=num_samples,init_lengthscale=init_lscale,cnntype=cnntype).cuda()
        lossfun = compute_loss_gp
        #opt = torch.optim.Adam(model.parameters(), lr=lr,weight_decay = weight_decay)
...
#saved_modelparam_path = './param_{}/dep{}_{}_{}_initl{}_{}_nsamples{}_nepochs{}.pth'.format(tasktype,dep,testtype,model.modelname,init_lscale,cnntype,num_samples,args.nepochs)

#save_dir = './param_{}/'.format(tasktype)
save_dir = './regression_task/param_{}/'.format(tasktype)
...
wandb.init( project="uai22-9234-synthetic",
...
for i in range(1,args.nepochs + 1):   
    ...
    if i%args.printfreq ==0 or i== 1:                
        #print('model.gpsampler.prior_scale {:.2f}'.format(model.gpsampler.prior_scale))

        #if tasktype == 'sin3':
        #    save_path_set = './syndata_{}_v{}/dep{}_{}_{}'.format(tasktype,args.datav, dep, testtype, -64)
        #if tasktype == 'mosm':
        #    #save_path_set = './syndata_{}_v{}/dep{}_{}_{}'.format(tasktype,args.datav, dep, testtype, -32)
        #    save_path_set = './syndata_{}_v{}/dep{}_{}_{}'.format(tasktype,args.datav, dep, testtype, -64)
        #save_path_set = './syndata_{}_v{}/dep{}_{}_{}'.format(tasktype,args.datav, dep, testtype, -64)
        save_path_set = './regression_task/syndata_{}_v{}/dep{}_{}_{}'.format(tasktype,args.datav, dep, testtype, -64)
