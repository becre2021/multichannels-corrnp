...
class DataGenerator(metaclass=abc.ABCMeta):
    ...
    def generate_task(self):
        ...
        task = {k: torch.tensor(_uprank(np.stack(v, axis=0)),dtype=torch.float32).to(device) for k, v in task.items()}
...
class SawtoothGenerator(DataGenerator):
    ...
    def sample(self, x):
        ...
        return 0.5 * amp - amp / np.pi * np.sum((-1) ** k * np.sin(2 * np.pi * k * freq * x) / k, axis=1)
