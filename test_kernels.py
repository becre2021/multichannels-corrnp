...
class SM_kernel(Multioutput_kernel):
    def __init__(self,in_dims=1,num_channels=3, scales=0.1,loglik_err=0.1):
        ...
        #self.logmu =    nn.Parameter( torch.log(eps + 0.05 + scales*torch.ones(in_dims,num_channels)) )
        
        #self.logmu =    nn.Parameter( torch.log(eps  + 0.0*torch.ones(in_dims,num_channels)) )                           
        ...
        #self.logmu =    nn.Parameter( torch.log(eps  + 0.01*torch.rand(in_dims,num_channels)) )                        
        #self.logstd =   nn.Parameter( torch.log(eps + scales*torch.rand(in_dims,num_channels)) )    
        #self.logmu =    nn.Parameter( torch.log(eps + scales*torch.rand(in_dims,num_channels)) )                      
        #self.logstd =   nn.Parameter( torch.log(eps + scales*torch.rand(in_dims,num_channels)) )                                    
...
#                 determinant = (pi2**self.in_dims)*((std_ji+1)**2).prod()                
#                 determinant = torch.sqrt(determinant)
#                 weight = (1/determinant)*torch.exp(-0.5*exp_in.sum())*weight0_                   
#                 #weight *= .25                
#             else:
#                 weight = weight0_
    ...
    def prepare_cross_params(self,eps=1e-6):
        ...
        for (j,i) in self.product_list:
            if j!=i:            
                ...
                weight_ji0 = torch.sqrt(weight[j]*weight[i])
                weight_ji = (1/determinant)*torch.exp(-0.5*exp_in.sum())*weight_ji0                   
                #weight *= .25                
            else:
                mu_ji = mu[:,j]
                std_ji = std[:,j]
...
#         self.logsigma = nn.Parameter( torch.log(eps + torch.ones(num_channels)) ,requires_grad=False)   
#         #self.logmu =    nn.Parameter( torch.log(eps + 0.01 + 0*torch.rand(in_dims,num_channels))  ,requires_grad=False)            
#         #self.logmu = nn.Parameter( torch.log(eps  + 0.0*torch.rand(in_dims,num_channels)) )              #nparams                
#         #self.logstd =   nn.Parameter( torch.log(eps + 0.1 + scales*torch.ones(in_dims,num_channels)) )    #nparams                         

#         self.logmu = nn.Parameter(    torch.log(eps + scales*torch.rand(in_dims,num_channels)) )              #nparams                
